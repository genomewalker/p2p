...
We explore the counts for each sample after the filtering and remove two smples with less than 10,000 counts
...
Now we will learn how to transform our data using the CSS transformation, for this we will create our own function:
...
We defined as endemic OTUs the ones observed in less than 20% of the samples (11) and with an abundance (counts) larger than 50. Ubiquitous are those observed in more than 80% of the samples


The results are quite interesting and fitting our other analyses where we see a large heterogeneity in the OSD2014 coastal sites. Few OTUs are ubiquitous and they are not too abundant.   

Endemic organisms

Note: I am not an expert in Eukaryotic taxonomic, I just selected the level where I saw the Diatoms
...
```{r}
ms174_df_subset <- as.data.frame(otu_table(ms174_physeq_filt_prev)[sample(1:303, 50),])
